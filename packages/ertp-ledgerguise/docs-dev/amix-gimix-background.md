...
- A requester (maintainer or sponsor) to post a _work agreement_ tied to a GitHub issue
...
The design is explicitly inspired by **Basic AMIX Consulting**, where economic exchange is modeled as a _state machine_ with explicit transitions.
...
- Funds are escrowed
- Acceptance criteria are fixed _implicitly_ by referencing:
...
- In GiMiX, this is _not_ an explicit on-chain action
...
- The chain does _not_ decide code quality
- The chain does _not_ mediate disputes
...
3. Uses an existing platform (e.g., GitHub, GitLab) as the _acceptance oracle_
...
The novelty is not the contract—it is the _delegation of acceptance authority_ and the disciplined separation of concerns.
